  ...
  async loadProductsByCategory(category) {
    ...
    try {
      ...
      if (subcategories.length > 1) {
        ...
        this.renderSubcategories(category, subcategories) this.renderProducts(products, category)
      } else {
  ...
  renderSubcategories(category, subcategories) {
    ...
    container.innerHTML = `
      <div class="subcategories-container">     
        ...
        <button class="ver-todos" data-action="back-to-category">← Volver a categorías</button>
      </div>
    `
    ...
    container.addEventListener('click', async (e) => {
      ...
      if (e.target.dataset.action === 'back-to-category') {
        await this.loadAllProducts()
      }
    })
  }
    ...
    if (products.length === 0) {
      container.innerHTML = `
        <div class="no-products">
          <h4>No hay productos disponibles en ${title}</h4>
